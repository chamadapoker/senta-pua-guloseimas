...
inventario.post('/', async (c) => {
  const { nome, finalidade, quantidade, unidade, valor_compra, fornecedor_id, observacao } = await c.req.json();
  if (!nome) return c.json({ error: 'Nome do item obrigatório' }, 400);
  const { results } = await c.env.DB.prepare(
    'INSERT INTO inventario (nome, finalidade, quantidade, unidade, valor_compra, fornecedor_id, observacao) VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING *'
  ).bind(
    nome.trim(), finalidade || null, quantidade ?? 0, unidade || 'un',
    valor_compra ?? null, fornecedor_id || null, observacao?.trim() || null
  ).all();
  return c.json(results[0], 201);
});
