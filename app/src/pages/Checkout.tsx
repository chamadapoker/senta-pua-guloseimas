...
export function Checkout() {
  ...
  return (
    <PublicLayout>
      <h1 className="font-display text-2xl text-white tracking-wider mb-5">FECHAR PEDIDO</h1>

      <div className="space-y-2 mb-6">
        {itens.map(({ produto, quantidade }) => (
          <div key={produto.id} className="bg-fundo-card rounded-xl p-3 flex items-center gap-3 border border-borda">
            {resolveImg(produto.imagem_url) ? (
              ...
              <div className="w-12 h-12 rounded-lg bg-fundo-elevado flex items-center justify-center text-xl">{produto.emoji}</div>
            )}
            <div className="flex-1 min-w-0">
              <div className="font-medium text-sm text-texto truncate">{produto.nome}</div>
              <div className="text-dourado text-sm font-bold">R$ {(produto.preco * quantidade).toFixed(2)}</div>
            </div>
            <div className="flex items-center gap-1.5">
              <button onClick={() => alterarQuantidade(produto.id, quantidade - 1)} className="w-7 h-7 rounded-lg bg-fundo-elevado text-texto border border-borda text-sm font-bold">-</button>
              <span className="w-6 text-center font-medium text-sm">{quantidade}</span>
              <button onClick={() => alterarQuantidade(produto.id, quantidade + 1)} className="w-7 h-7 rounded-lg bg-fundo-elevado text-texto border border-borda text-sm font-bold">+</button>
              <button onClick={() => remover(produto.id)} className="ml-1 text-texto-fraco hover:text-vermelho text-lg">&times;</button>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="space-y-4 mb-6">
        ...
        <div>
          ...
          <input
            ...
            className="w-full bg-fundo-card border border-borda rounded-xl px-4 py-3 text-base text-texto placeholder-texto-fraco/50 focus:outline-none focus:ring-2 focus:ring-dourado/50 focus:border-dourado/50"
          />
        </div>
      </div>

      {erro && <p className="text-vermelho text-sm mb-3 bg-vermelho/10 border border-vermelho/20 rounded-lg px-3 py-2">{erro}</p>}
